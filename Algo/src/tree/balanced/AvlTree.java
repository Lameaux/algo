...
/**
 * Implementation of AVL Balanced BST. Self-balancing tree. Key-value storage.
 * 
 * @author sergii.sizov
 * 
 * @param <K>
...
public class AvlTree<K extends Comparable<K>, V> implements BalancedBST<K, V> {
	...
	private AvlTreeNode<K, V> balanceNodeAfterInsert(AvlTreeNode<K, V> node, K key) {
		...
		// Left Left - key was inserted into left left subtree, x ~
		// node.left.left
		if (balance > 1 && Objects.less(key, node.left.key)) {
			return rotateRight(node);
		}

		// Right Right - key was inserted into right right subtree, x ~
		// node.right.right
		if (balance < -1 && Objects.greater(key, node.right.key)) {
			return rotateLeft(node);
		}
		// Left Right - double rotation - key was inserted into left right
		// subtree, x ~ node.left.right
		if (balance > 1 && Objects.greater(key, node.left.key)) {
			node.left = rotateLeft(node.left); // left rotate right part
			return rotateRight(node);
		}

		// Right Left - double rotation - key was inserted into right left
		// subtree, x ~ node.right.left
...
/**
 * Node of the AVL Tree. Contains key->value, pointers to left and right
 * children, calculated height of the subtree.
 * 
 * @author sergii.sizov
 * 
 * @param <K>
